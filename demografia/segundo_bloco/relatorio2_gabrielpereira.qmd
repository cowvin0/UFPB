...
format: pdf
title: |
  Comparando indicadores demográficos entre Brasil e Rio de Janeiro (censo de 2000, 2010 e 2022)
...
```{python}
pop_brasil_rj_2022 = pd\
  .read_excel('pop_brasil_rj_2022.xlsx')\
  .drop(
    columns=[
...
juntos_urbano_rural_2000.astype(int).style.hide()
```
...
juntos_urbano_rural_2010.astype(int).style.hide()
```
...
A tabela abaixo apresenta a razão de sexo para o Brasil e o estado do Rio de Janeiro em 2022. A razão de sexo mostra a proporção de homens para cada 100 mulheres. No Brasil, a razão de sexo é de 94,25, indicando que há cerca de 94 homens para cada 100 mulheres. No Rio de Janeiro, essa razão é ainda menor, de 89,39, o que significa que há aproximadamente 89 homens para cada 100 mulheres no estado.

```{python}
pop_brasil_rj_2022\
  .groupby(['Unidade da Federação', 'Sexo'])\
  [['Frequência']]\
  .sum()\
  .unstack(1)\
  .reset_index()\
  .assign(
    Razão_de_Sexo=lambda x: x[('Frequência', 'Homens')] / x[('Frequência', 'Mulheres')] * 100
  )\
  .rename(
    columns={'Razão_de_Sexo': 'Razão de Sexo'}
  )\
  .style.hide()
```

Esses valores sugerem que, em ambas as regiões, o número de mulheres é maior que o de homens, com uma diferença mais acentuada no Rio de Janeiro. Essa disparidade pode ser explicada por diversos fatores, como maior mortalidade masculina, migração ou até diferenças na expectativa de vida, em que mulheres tendem a viver mais do que homens, especialmente em faixas etárias mais avançadas.
...
pop_brasil_rj_2022['Intervalo de Idade'] = pd.Categorical(pop_brasil_rj_2022['Intervalo de Idade'], categories=categorias)
mulher_etario_2022 = pop_brasil_rj_2022.query('Sexo == "Mulheres"')
homem_etario_2022 = pop_brasil_rj_2022.query('Sexo == "Homens"')
```
...
dep_total_2022 = pd.DataFrame({
  'Ano': [2022] * 4,
  'Unidade da Federação': ['Rio de Janeiro', 'Brasil'] * 2,
  'Sexo': ['Feminino', 'Feminino', 'Masculino', 'Masculino'],
  'RDT': [calcular_rdt(mulher_etario_2022, 'Rio de Janeiro'),
          calcular_rdt(mulher_etario_2022, 'Brasil'),
          calcular_rdt(homem_etario_2022, 'Rio de Janeiro'),
          calcular_rdt(homem_etario_2022, 'Brasil')],
  'RDJ': [calcular_rdj(mulher_etario_2022, 'Rio de Janeiro'),
          calcular_rdj(mulher_etario_2022, 'Brasil'),
          calcular_rdj(homem_etario_2022, 'Rio de Janeiro'),
          calcular_rdj(homem_etario_2022, 'Brasil')],
  'RDI': [calcular_rdi(mulher_etario_2022, 'Rio de Janeiro'),
          calcular_rdi(mulher_etario_2022, 'Brasil'),
          calcular_rdi(homem_etario_2022, 'Rio de Janeiro'),
          calcular_rdi(homem_etario_2022, 'Brasil')],
          })

pd.concat([dep_total_2022, dep_total_2010, dep_total_2000], axis=0).style.hide()
```
...
\ \ \ Em 2022, tanto no Brasil quanto no Rio de Janeiro, a razão de dependência total (RDT) mostra que, para cada 100 pessoas em idade ativa (15 a 64 anos), há aproximadamente 35 a 37 pessoas dependentes, entre jovens e idosos. No caso das mulheres, a RDT no Brasil é de 36,44, com uma predominância da dependência jovem (RDJ de 25,66) e uma razão de dependência idosa (RDI) de 10,78. No Rio de Janeiro, essa razão total é um pouco menor, 35,62, mas com uma dependência jovem de 22,41 e uma RDI de 13,20. Isso indica que as mulheres no Rio de Janeiro possuem uma menor dependência de jovens, mas uma maior proporção de idosos dependentes em comparação com o Brasil.

\vspace{12pt}

\ \ \ Entre os homens, o cenário é semelhante, embora com números ligeiramente diferentes. No Brasil, a RDT masculina é de 36,78, com uma dependência jovem de 28,41 e uma RDI de 8,37. Já no Rio de Janeiro, a RDT dos homens é de 35,30, com uma dependência jovem de 25,98 e uma RDI de 9,32. Isso mostra que, também entre os homens, a dependência idosa no Rio de Janeiro é um pouco maior que no Brasil, mas a dependência jovem é menor.
...
velhice_total_2022 = pd.DataFrame({
  'Ano': [2022] * 4,
  'Unidade da Federação': ['Rio de Janeiro', 'Brasil'] * 2,
  'Sexo': ['Feminino', 'Feminino', 'Masculino', 'Masculino'],
  'Índice de Envelhecimento': [calcular_ie(mulher_etario_2022, 'Rio de Janeiro'),
          calcular_ie(mulher_etario_2022, 'Brasil'),
          calcular_ie(homem_etario_2022, 'Rio de Janeiro'),
          calcular_ie(homem_etario_2022, 'Brasil')],
          })

pd.concat([velhice_total_2022, velhice_total_2010, velhice_total_2000], axis=0).style.hide()
```
...
\ \ \ Considerando o Brasil em 2022, o IE para mulheres é 90,97, o que indica que há aproximadamente 91 mulheres idosas para cada 100 jovens. Este valor é mais baixo que o do Rio de Janeiro, mas ainda mostra um envelhecimento crescente. Para os homens, o índice é 69,51, também menor que no Rio de Janeiro, mas com um padrão semelhante de crescimento.

\vspace{12pt}

\ \ \ Agora para o Rio de Janeiro em 2022, o IE para mulheres é 125,84, indicando que, para cada 100 jovens com menos de 15 anos, há aproximadamente 126 mulheres com 60 anos ou mais. Isso sugere um envelhecimento populacional significativo no estado. Para os homens, o índice é de 86,76, ou seja, há cerca de 87 homens idosos para cada 100 jovens, o que também indica um aumento considerável no envelhecimento masculino.
...
$$
M_d = 20 + \left(\frac{41.788.007,5 -  34.526.048}{14.862.546}\right) \times 9
$$
...
$$
M_d = 30 + \left(\frac{3.745.473,5 - 3.700.536}{1.182.198}\right) \times 9 = 30.34211
$$
...
$$
M_d = 20 + \left(\frac{43.111.577,5 - 33.679.889}{8.920.685}\right) \times 9 = 29,51555
$$
...
\ \ \ A partir das tabelas, a idade mediana para os homens no estado do Rio de Janeiro em 2010 é de 31,07598 anos, enquanto para o Brasil é de 27,78193 anos. Para as mulheres no Rio de Janeiro, a idade mediana em 2010 é de 33,62949 anos, comparada a 30,2485 anos para o Brasil. A população masculina do Rio de Janeiro apresentou um aumento na idade mediana bastante significativa. Para as mulheres, houve um aumento ainda maior na idade mediana no Brasil.

\vspace{12pt}

\ \ \ Agora seguinda da mesma forma para o cálcula da idade mediana por sexo para o estado do Rio de Janeiro e o Brasil como um todo no ano de 2022, temos as tabelas abaixo que representam o sexo masculino e feminino, respectivamente:


```{python}
homem_etario_2022\
  .sort_values(['Unidade da Federação', 'Intervalo de Idade'])\
  .assign(
    Acumulado=lambda x: x.groupby('Unidade da Federação').Frequência.transform('cumsum')
  )\
  .style.hide()
```
...
Em 2022, tanto para a população brasileira quanto para a do estado do Rio de Janeiro, foi possível observar um envelhecimento populacional ainda mais acentuado, especialmente entre as mulheres do Rio de Janeiro. A idade mediana dos homens no estado foi de 35,50 anos, enquanto para o Brasil foi de 33,57 anos. No caso das mulheres, o aumento da idade mediana foi ainda mais expressivo, chegando a 38,36 anos no Rio de Janeiro, em comparação a 35,53 anos para o Brasil.
...
\ \ \ Os últimos dois gráficos são as pirâmides etárias para o Brasil e estado do Rio de Janeiro, respectivamente. A partir dos gráficos, é possível perceber claramente o envelhecimento da população, o que só ficou mais claro em relação à pirâmide etária do Rio de Janeiro no ano de 2010. Esse envelhecimento é mais acentuado ainda em relação ao Rio de Janeiro, principalmente em relação ao sexo feminino. O envelhecimento do sexo feminino também é o mais acentuado em relação a pirâmide etária brasileira.
...
piram_2000_brasil = pd.concat([piram_muie_2000_brasil, piram_homi_2000_brasil.drop(columns=['Intervalo de Idade'])], axis=1)\
  .rename(columns={'Intervalo de Idade': 'Idade', 'Unidade da Federação': 'UF'})\
  .drop(columns=['UF'])\
  .sort_values('Idade', ascending=False)
```
...
piram_2010_brasil = pd.concat([piram_muie_2010_brasil, piram_homi_2010_brasil.drop(columns=['Intervalo de Idade'])], axis=1)\
  .rename(columns={'Intervalo de Idade': 'Idade', 'Unidade da Federação': 'UF'})\
  .drop(columns=['UF'])\
  .sort_values('Idade', ascending=False)
```
...
```{python}
piram_muie_2022_brasil = mulher_etario_2022\
  .rename(columns={'Frequência': 'Feminino'})\
  .query('`Unidade da Federação` == "Brasil"')\
  .rename(columns={'Intervalo de Idade': 'Idade'})\
  .sort_values('Idade', ascending=False)\
  .drop(columns=['Sexo'])\
  .reset_index(drop=True)

piram_homi_2022_brasil = homem_etario_2022\
  .rename(columns={'Frequência': 'Masculino'})\
  .query('`Unidade da Federação` == "Brasil"')\
  .rename(columns={'Intervalo de Idade': 'Idade'})\
  .sort_values('Idade', ascending=False)\
...
piram_2022_brasil = piram_muie_2022_brasil\
  .merge(piram_homi_2022_brasil, on=['Unidade da Federação', 'Idade'])
```

```{python}
piram_muie_2022 = mulher_etario_2022\
  .rename(columns={'Frequência': 'Feminino'})\
  .query('`Unidade da Federação` == "Rio de Janeiro"')\
  .rename(columns={'Intervalo de Idade': 'Idade'})\
  .sort_values('Idade', ascending=False)\
...
piram_homi_2022 = homem_etario_2022\
  .rename(columns={'Frequência': 'Masculino'})\
  .query('`Unidade da Federação` == "Rio de Janeiro"')\
  .rename(columns={'Intervalo de Idade': 'Idade'})\
  .sort_values('Idade', ascending=False)\
...
piram_2022 = piram_muie_2022\
  .merge(piram_homi_2022, on=['Unidade da Federação', 'Idade'])
```

::: {#fig-py2 layout-ncol=2}

```{python}
#| label: fig-pir2022_brasil

barplot = sns.barplot(
  data=piram_2022_brasil.assign(Masculino=lambda x: -x.Masculino),
  x="Masculino",
  y="Idade",
  order=piram_2022_brasil['Idade'],
  color="#049899",
  lw=0
  )
...
barplot = sns.barplot(
  x="Feminino",
  y="Idade",
  data=piram_2022_brasil.sort_values('Idade'),
  order=piram_2022_brasil.Idade.sort_values(ascending=False).cat.categories,
  color="#ED9502",
  lw=0
)
plt.xlabel('População')
...
```{python}
#| label: fig-pir2022_rj

barplot = sns.barplot(
  data=piram_2022.assign(Masculino=lambda x: -x.Masculino),
  x="Masculino",
  y="Idade",
  order=piram_2022['Idade'],
  color="#049899",
  lw=0
  )
...
barplot = sns.barplot(
  x="Feminino",
  y="Idade",
  data=piram_2022.sort_values('Idade'),
  order=piram_2022.Idade.sort_values(ascending=False).cat.categories,
  color="#ED9502",
  lw=0
)
plt.xlabel('População')
...
Gráfico de pirâmide da população no Brasil e Rio de Janeiro. O gráfico na figura à esquerda é a pirâmide etária no Brasil em 2022. O da direita é do Rio de Janeiro, ano de 2022.
:::
...
O Índice de Myers não foi calculado para o ano de 2022, pois o censo desse ano ainda não disponibilizou os dados de população por idades detalhadas.

# Conclusão

 \ \ \ A partir das comparações entre o estado do Rio de Janeiro e o Brasil, observou-se que, de 2010 a 2022, o Rio de Janeiro seguiu um padrão muito similar ao do país em todos os indicadores analisados. No entanto, no censo de 2022, começa a se destacar com diferenças mais pronunciadas. Em relação às pirâmides etárias, as análises mostram que, entre 2000 e 2010, as diferenças entre as duas regiões não eram muito marcantes. Contudo, em 2022, a pirâmide etária do Rio de Janeiro apresenta um afunilamento mais visível, indicando um envelhecimento mais expressivo da população. O mesmo padrão se reflete na idade mediana, que já mostrava sinais de envelhecimento em 2010, especialmente entre as mulheres. Em 2022, esse envelhecimento se acentuou significativamente no Rio de Janeiro, especialmente entre as mulheres, com a idade mediana se aproximando de 39 anos.
