...
\ \ \ As análises apresentadas neste estudo foram realizadas utilizando a linguagem de programação R [@rlang], com o auxílio de todo ecossistema Tidyverse [@tidy] para manipulação de dados e do pacote ggplot [@ggplot] para visualização gráfica. Os documentos do relatório foram elaborados com o Quarto [@quarto], um sistema de escrita e publicação científica. Todo o código-fonte utilizado nas análises está disponível no GitHub [@github_rel3].
...
Existem diversas taxas específicas de mortalidade. No entanto, nesse trabalho foi utilizado a taxa especifica de mortalidade por faixa etária e por algumas causas específicas de óbito, que é expressa da seguinte forma:
...
pop_br_2010 <- tibble(
  ...
  pivot_longer(
    ...
    names_to = "Sexo",
    values_to = "População Brasil - 2010"
  )
```


```{r}
pop_br_2022 <- readxl::read_xlsx('dados_relatorio3/pop_br_2022.xlsx') |>
  mutate(
    `1 a 4 anos` = `0 a 4 anos` - `Menos de 1 ano`,
    `20 a 29 anos` = `20 a 24 anos` + `25 a 29 anos`,
    `30 a 39 anos` = `30 a 34 anos` + `35 a 39 anos`,
  ...
  pivot_longer(
    cols = contains("ano"),
    names_to = "Idade",
    values_to = "População Brasil - 2022"
  ) |>
  left_join(pop_br_2010, by = c("Sexo", "Idade")) |>
  left_join(
    tme |>
      select(c(Sexo, Idade, `TME 2010`, `TME 2022`)),
      by = c("Sexo", "Idade")
  )
...
pop_br_2022 |>
  group_by(Sexo) |>
  summarise(
    `O_esp 2010` = sum(`TME 2010` * `População Brasil - 2010`),
    `O_esp 2022` = sum(`TME 2022` * `População Brasil - 2022`),
    `Total População 2010` = sum(`População Brasil - 2010`),
    `Total População 2022` = sum(`População Brasil - 2022`),
    `TBM 2010 - Padronizada` = `O_esp 2010` / `Total População 2010` * 1000,
    `TBM 2022 - Padronizada` = `O_esp 2022` / `Total População 2022` * 1000,
  ) |>
  knitr::kable()
```
...
```{r}
mortalidade_neonatal <- tibble(
  Sexo = c("Homens", "Mulheres"),
  `2010` = c(1082, 920),
  `2022` = c(832, 691)
)
mortalidade_neonatal |> knitr::kable()
```
...
```{r}
tabela_neonatal_precoce <- tibble(
  Sexo = c("Homens", "Mulheres"),
  `2010` = c(825, 710),
  `2022` = c(628, 496)
)

tabela_neonatal_precoce |>
  knitr::kable()
```
...
```{r}
tabela_neonatal_tardia <- tibble(
  Sexo = c("Homens", "Mulheres"),
  `2010` = c(257, 210),
  `2022` = c(204, 195)
)
tabela_neonatal_tardia |> knitr::kable()
```
...
```{r}
tabela_pos_neonatal <- tibble(
  Sexo = c("Homens", "Mulheres"),
  `2010` = c(544, 451),
  `2022` = c(442, 402)
)
tabela_pos_neonatal |> knitr::kable()
```
...
```{r}
tabela_perinatal <- tibble(
  Sexo = c("Homens", "Mulheres"),
  `2010` = c(1365 + 825, 1167 + 710),
  `2022` = c(1134 + 628, 1005 + 496)
)
tabela_perinatal |> knitr::kable()
```
...
tmp <- tibble(
  Sexo = c("Homens", "Mulheres"),
  `2010` = c((1365 + 825) / 110269 * 1000, (1167 + 710) / 104944 * 1000),
  `2022` = c((1134 + 628) / 92055 * 1000, (1005 + 496) / 88309 * 1000)
)
...
tibble(
  Ano = c(2010, 2022),
  TMM = c(180 / (104944 + 110269) * 100000, 125 / (88309 + 92055) * 100000)
) |>
  knitr::kable()
```
...
tme |>
  group_by(Sexo) |>
  summarize(
    `Total População 2010` = sum(`População 2010`),
    `Total População 2022` = sum(`População 2022`)
  ) |>
  knitr::kable()
```
...
causas_obs <- tibble(
  ...
  Ano = rep(2022, 2),
  Mulheres = c(19128, 3262),
  Homens = c(19397, 9362),
  `TMC - Homens` = Homens / 7577675 * 1000,
  `TMC - Mulheres` = Mulheres / 8477499 * 1000
)

causas_obs_2010 <- tibble(
  Causa = c(
    "Doenças do aparelho circulatório",
    "Causas externas de morbidade e mortalidade"
  ),
  Ano = rep(2010, 2),
  Mulheres = c(18128, 2925),
  Homens = c(19397, 9362),
  `TMC - Homens` = Homens / 7625679 * 1000,
  `TMC - Mulheres` = Mulheres / 8364250 * 1000
)

causas_obs <- rbind(causas_obs, causas_obs_2010)

causas_obs |> knitr::kable()
```
...
pop_total <- tibble(
  Sexo = c("Homens", "Mulheres"),
  `2010` = c(7625679, 8364250),
  `2022` = c(7577675, 8477499)
)
...
tbn <- tibble(
  Sexo = c("Homens", "Mulheres"),
  `2010` = c(110269 / (7625679 + 8364250) * 1000, 104944 / (7625679 + 8364250) * 1000),
  `2022` = c(92055 / (7577675 + 8477499) * 1000, 88309 / (7577675 + 8477499) * 1000)
)
tbn |> knitr::kable()
```
...
tfg <- tibble(
  Sexo = c("Homens", "Mulheres"),
  `2010` = c(110269 / 4482651 * 1000, 104944 / 4482651 * 1000),
  `2022` = c(92055 / 4196083 * 1000, 88309 / 4196083 * 1000)
)
tfg |> knitr::kable()
```
...
idade_mae_feminino <- tibble(
  ...
  `2022 mães` = c(
    9511, 21569 + 22364,
    18321 + 12066, 3810 + 260
    )
)

idade_mae_masculino <- tibble(
  `Idade` = c(
    "15 a 19 anos", "20 a 29 anos",
    "30 a 39 anos", "40 a 49 anos"
  ),
  ...
  `2010 mães` = c(
    18916, 28339 + 27964,
    20946 + 10497, 2598 + 161
    ) ,
  `2022 mães` = c(
    9841, 22461 + 23164,
    19201 + 12617, 4069 + 287
    )
)

pop_2010 <- df_populacao_2010 |>
  filter(Sexo == "Mulheres") |>
  filter(
    Idade %in% c(
      "15 a 19 anos", "20 a 29 anos",
  ...
  rename(`2010 mulheres` = População) |>
  select(-Sexo)

pop_2022 <- df_populacao |>
  filter(Sexo == "Mulheres") |>
  filter(
    Idade %in% c(
      "15 a 19 anos", "20 a 29 anos",
  ...
  rename(`2022 mulheres` = População) |>
  select(-Sexo)

pop <- left_join(pop_2010, pop_2022)

idade_mae <- rbind(idade_mae_feminino, idade_mae_masculino)

tefs <- left_join(idade_mae, pop) |>
  mutate(
    `TEF 2010` = `2010 mães` / `2010 mulheres` * 1000,
    `TEF 2022` = `2022 mães` / `2022 mulheres` * 1000
  )

tefs |> knitr::kable()

# df_etaria_fecundidade_feminino <- read_csv2('dados_relatorio3/nascidos_vivos_faixaet_mae_feminino.csv')
# df_etaria_fecundidade_masculino <- read_csv2('dados_relatorio3/nascidos_vivos_faixaet_mae_masculino.csv')

# df_etaria_fecundidade_feminino
# df_etaria_fecundidade_masculino
```

A partir da tabela, é possível ver claramente que a taxa de fecundidade específica diminuiu para a maioria das idades, indicando aquele padrão que foi visto analisando os outros indicadores de fecundidade. No entanto, houve um aumento na taxa de fecundidade específica entre as idades de 30 a 39 anos e de 40 a 49 anos, indicando que agora as mulheres possam estar pretendendo ter filhos mais tarde devido a melhores condições de trabalho e até mesmo maiores oportunidades de estudo.
...
<!-- A tabela apresenta a taxa de fecundidade total (TFT) no estado do Rio de Janeiro nos anos de 2010 e 2022, considerando o sexo da criança. Em 2010, a taxa de fecundidade para nascimentos masculinos foi de 0.8285, enquanto para nascimentos femininos foi de 0.7880. Já em 2022, esses valores caíram para 0.0874 para meninos e 0.0839 para meninas. Isso indica uma queda acentuada nas taxas de fecundidade total ao longo do período, sugerindo uma diminuição significativa no número de nascimentos. Essa redução pode estar relacionada a fatores como mudanças no comportamento reprodutivo, maior acesso a métodos contraceptivos, ou transformações socioeconômicas e demográficas no estado. -->

```{r}
tefs2 <- tefs |>
  group_by(Idade) |>
  summarise(
    `2010 mães` = sum(`2010 mães`),
    `2022 mães` = sum(`2022 mães`),
    `2010 mulheres` = sum(`2010 mulheres`),
    `2022 mulheres` = sum(`2022 mulheres`),
  ) |>
  mutate(
    `TEF 2010` = `2010 mães` / `2010 mulheres`,
    `TEF 2022` = `2022 mães` / `2022 mulheres`,
    n = case_when(
      Idade == "15 a 19 anos" ~ 5,
      TRUE ~ 10
    ),
  )
...
tefs2 |>
  summarise(
    `TFT 2010` = sum(n * `TEF 2010`),
    `TFT 2022` = sum(n * `TEF 2022`),
  ) |>
  knitr::kable()
```
...
# ) |>
#   mutate(
#     `TFT 2010` = `TFT 2010` / 1000,
#     `TFT 2022` = `TFT 2022` / 1000
#   )
...
df_tabuanormal <- df_tabuanormal |>
  ...
  mutate(
    ...
    ex = Tx / Ix) |>
  select(-c(amplitude, ax, qi, Mx, M0, dx, Óbitos_externos))
  # select(-c(qi, dx, ax, amplitude, M0, Mx, Tx, Lx, Ix))

df_tabuanormal |> mutate(across(is.numeric, \(x) round(x, 3))) |>
 knitr::kable()
```
...
df <- df |>
  ...
  # select(-c(pi, qi, dx, ax, amplitude, M0, Mx, desconto, Tx, Lx, Ix))
