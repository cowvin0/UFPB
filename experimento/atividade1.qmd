...
Mais da metade dos testes da @tbl-norm não rejeitam a hipótese de normalidade, e gráficamente pelo Q-Q plot da @fig-norm os resíduos parecem sim ter distribuição aproximadamente normal pois se assemelham aos quantis teóricos da distribuição normal.
...
Todos os testes da @tbl-homo não rejeitam a hipótese de que as variâncias são iguais entre os grupos, com p-valores acima de 5%. Isso indica que a variância do modelo é constante para todos os valores estimados da variável resposta, o gráfico da @fig-homo apoia esta suposição.

```{r}
#| fig-cap: Gráfico dos resíduos padronizados versus valores estimados
#| label: fig-homo
#| fig-height: 4
#| out-width: 100%

# Código para criação de gráfico dos resíduos padronizados 
# versus valores estimados

modelo |> 
  augment(newdata = data) |>
...
Agora calculando: $S_{\bar y_{i.}} = \sqrt \frac{MS_E}{n} = 56.625$
...
data |> 
  ...
  xlim(1500, 4000) +
  geom_function(fun = \(x) dt(x, df = 12, ncp = 2800) * 56.625, color = "#21908C") +
