...
Pelo gráfico de barras, podemos ver que a correlação entre as variáveis canônicas de $\left(U1, V1\right)$ é bastante correlacionada, chegando a 99.56%. Já para $\left(U2, V2\right)$ a correlação foi apenas de 19.71%, bastante baixa.
...
```{r echo=FALSE}
pvte.v <- (colSums((loadings$corr.X.xscores)^2))/(dim(X)[2])*100
barplot(
  pvte.v / sum(pvte.v),
  xlab = "Variável",
  ylab = "Variância total explicada",
...
cm <- confusionMatrix(test$Sex, predict(linear, test)$class)
```
