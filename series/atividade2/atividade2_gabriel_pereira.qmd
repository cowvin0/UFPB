...
<!-- # dados_deflacionado <- tibble( -->
<!-- #   Data = seq(as.Date("1997-01-01"), as.Date("2019-04-01"), by = "months"), -->
<!-- #   ICMS = deflate(dados$ICMS, dados$Data, "04/2019", "igpm"), -->
<!-- #   Primário = deflate(dados$Primário, dados$Data, "04/2019", "igpm"), -->
<!-- #   Secundário = deflate(dados$Secundário, dados$Data, "04/2019", "igpm"), -->
<!-- #   Terciário = deflate(dados$Terciário, dados$Data, "04/2019", "igpm"), -->
<!-- #   Energia = deflate(dados$Energia, dados$Data, "04/2019", "igpm"), -->
<!-- #   Petróleo = deflate(dados$Petróleo, dados$Data, "04/2019", "igpm"), -->
<!-- #   Transferências = deflate(dados$Transferências, dados$Data, "04/2019", "igpm"), -->
<!-- #   Exportação = deflate(dados$Exportação, dados$Data, "04/2019", "igpm"), -->
<!-- #   Importação = deflate(dados$Importação, dados$Data, "04/2019", "igpm"), -->
<!-- #   `Alta do Mês` = dados$`Alta do Mês`, -->
<!-- #   `Alta do Trimestre` = dados$`Alta do Trimestre` -->
<!-- #   ) -->
...
```{r output=FALSE}
dados_deflacionado |> 
  select(-c(Exportação, Transferências, Importação)) |> 
  pivot_longer(
    -Data,
    names_to = "Série",
    values_to = "Reais"
...
Assim, as médias móveis não consegue ponderar as observações mais antigas, além também de não conseguir captar a tendência e sazonalidade.
...
O método de Médias Móveis Simples não se ajusta bem à série, pois não pondera as observações mais antigas. Além disso, não consegue captar nem a tendência e nem a sazonalidade que estão presenta na série.
...
Os modelos ARIMA/SARIMA, diferente dos algoritmos de alisamento exponencial que são baseados numa descrição da tendência e sazonalidade contida na série, os modelos ARIMA procuram descrever as autocorrelações.
...
Como aconteceu para os métodos anteriores, o SARIMA consegue fazer melhores previsões para os primeiros 6 meses. Conseguiu se ajustar um pouco melhor aos dados que o algoritmo de Holt-Wilters.
...
No caso da série de ICMS, foi estimado um modelo $ETS$ com erro aditivo, sem tendência e sazonalidade aditiva. Dessa forma, $ETS\left(A, N, A\right)$ Além disso, assim como no modelo $SARIMA$, foi feita uma transformação log na série de ICMS.
...
As métricas obtidas do modelo ETS foram as melhores agora, pois consegue captar bem as componentes do erro e sazonalidade, que são as mais representativas na série.
...
As melhores previsões para os períodos considerados foram obtidos pelo modelo ETS. O segundo melhor foi o MMS e o terceiro melhor foi o de Holt-Winters. Comparando o Holt-Winters com o SARIMA e SARIMAX, vemos que ele produz previsões piores para os primeiros 6 meses, mas tem previsões melhores que os dois para períodos maiores que 6 meses.
