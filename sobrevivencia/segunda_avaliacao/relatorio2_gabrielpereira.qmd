...
O modelo foi construído considerando os diferentes grupos de doença de base dos pacientes em diálise e também a idade dos pacientes. A partir dos dados da tabela a seguir, é possível ver que há indícios de que, ao nível de 5% de significância, apenas a variável idade é significativa, mas os grupos de doenção não são significativos.
...
plot(st_wei, sexp_wei, xlab = "S(ei): Kaplan-Meier", ylab = "S(ei): Weibull Padrão", pch = 16)
